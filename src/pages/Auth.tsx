...
import { Mail, Lock, User, Loader2, AlertTriangle, ShieldAlert, Phone, Send } from 'lucide-react';
...
const phoneSchema = z.string().regex(/^\+\d{9,15}$/, "Telefon raqam +998901234567 formatida bo'lishi kerak");
...
function AuthContent() {
  ...
  const handleSendOtp = async () => {
    ...
    if (!r.success) { setPhoneError(r.error.errors[0].message); return; }
    setPhoneError(null);
    setLoading(true);
  ...
  const handleVerifyOtp = async () => {
    if (otp.length < 4) { toast.error("Kodni kiriting"); return; }
    setLoading(true);
  ...
  const handleTelegram = () => {
    toast.info("Telegram orqali kirish tez orada qo'shiladi");
  };
          ...
          <CardContent className="space-y-4">
            ...
            <Tabs defaultValue="login" className="w-full">
              ...
              <TabsContent value="phone" className="space-y-4">
                ...
                {!otpSent ? (
                  <Button onClick={handleSendOtp} disabled={loading} className="w-full gradient-primary border-0">
                    {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                    SMS kod yuborish
                  </Button>
                ) : (
                  <div className="space-y-2">
                    <Button onClick={handleVerifyOtp} disabled={loading} className="w-full gradient-primary border-0">
                      {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                      Tasdiqlash va kirish
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => { setOtpSent(false); setOtp(''); }} className="w-full">
                      Telefon raqamini o'zgartirish
                    </Button>
                  </div>
                )}
                <p className="text-xs text-muted-foreground text-center">
                  Telefon raqam orqali kirish uchun SMS provayder Lovable Cloud sozlamalarida yoqilgan bo'lishi kerak.
                </p>
              </TabsContent>
            </Tabs>
          </CardContent>
